...
model_file_dict = {
    'SASRec': {
        ...
        'mind': {
            'None': 'SASRec-Nov-29-2025_10-52-24.pth',  # 기본 모델
            'category': 'SASRec_AddInfo2-Nov-29-2025_11-15-38.pth',  # 카테고리 기반 검색 도구
            'subcategory': 'SASRec_AddInfo2-Nov-29-2025_11-35-02.pth',  # 서브카테고리 기반 검색 도구
        },
        'mind_2000': {
...
model_BERT = {
    'SASRec': {
        ...
        'mind': {
            'category': 'SASRec_AddInfo2-Nov-29-2025_11-15-38.pth',
            'subcategory': 'SASRec_AddInfo2-Nov-29-2025_11-35-02.pth',
        },
        'mind_2000': {
            'category': 'SASRec_AddInfo2-mind_2000-category.pth',
            'subcategory': 'SASRec_AddInfo2-mind_2000-subcategory.pth',
...
# 동일 실사용자의 여러 테스트 행 → 가상 uid(`base__instk`) 사용 시 CRS 조회용 원래 user id
uid_alias_map = {}
_uid_alias_path = os.path.join(prompts_path, dataset_name + '_uid_alias.pkl')
if os.path.isfile(_uid_alias_path):
    with open(_uid_alias_path, 'rb') as f:
        uid_alias_map = pickle.load(f)


def resolve_dataset_uid(uid):
    """CRS/추천 엔진에 넘길 실제 user token (dump_to_chat 다중 인스턴스용)."""
    u = str(uid)
    return uid_alias_map.get(u, u)
...
def _item_id_known_in_token_map(iid: str, token_map: dict) -> bool:
    """RecBole field2token_id['item_id']: 키는 보통 N123 형태 또는 원시 토큰 문자열."""
    if iid in token_map:
        return True
    nt = f"N{iid}" if iid.isdigit() else None
    if nt and nt in token_map:
        return True
    return False


def extract_and_check_cur_user_reclist(ranked_str, topk=10, allowed_ids=None):
    """
    추천 리스트 형식 검증 함수
    Returns: 0 if valid, 1 if invalid

    allowed_ids:
        지정 시 각 ID가 이 집합에 속하는지만 검사 (LLM reranking 후보 고정 시 사용).
        None 이면 RecBole item_token_id 로 카탈로그 존재 여부 검사.
    """
    ...
    if allowed_ids is not None:
        invalid_ids = [i for i in cur_user_reclist if i not in allowed_ids]
        if invalid_ids:
            print(f"  [디버깅 extract_and_check] 허용 후보 집합 밖의 ID: {invalid_ids[:5]}")
            return 1
        return 0

    invalid_ids = []
    for iid in cur_user_reclist:
        if not _item_id_known_in_token_map(iid, item_token_id):
            invalid_ids.append(iid)

    if invalid_ids:
        print(f"  [디버깅 extract_and_check] item_token_id 에 없는 ID: {invalid_ids[:5]}")
        sample_keys = list(item_token_id.keys())[:5]
        print(f"  [디버깅 extract_and_check] item_token_id 샘플 키: {sample_keys}")

    return 1 if invalid_ids else 0
