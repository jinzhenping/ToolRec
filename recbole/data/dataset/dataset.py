...
class Dataset(torch.utils.data.Dataset):
    ...
    def dump_profile(self):
        ...
        if sample_size is not None and sample_size > 0:
            ...
            if actual_sample_size < len(users):
                ...
                uid_iid = {u: uid_iid[u] for u in sampled_users}
                uid_iid_his = {u: uid_iid_his.get(u, []) for u in sampled_users}
                uid_iid_hisScore = {u: uid_iid_hisScore.get(u, []) for u in sampled_users}
                self.logger.info(f"Sampled {actual_sample_size} users from {len(users)} total users")
            else:
                self.logger.info(f"Using all {len(users)} users (sample_size >= total users)")
        ...
        for index, item_row in item_df.iterrows():
            ...
            elif self.dataset_name in ('mind', 'mind_2000'):
                # 뉴스형: title, category, subcategory 사용
        ...
        if self.dataset_name == 'ml-1m':
            ...
            # 뉴스형: 사용자 프로필 생성 (score_pattern 없음)
            for uid in uid_iid.keys():
                user_description = patterns['user_pattern']
                user_info = ""
                for (his_item, his_score) in zip(uid_iid_his[uid], uid_iid_hisScore[uid]):
    ...
    def _load_inter_feat(self, token, dataset_path):
        ...
        if self.benchmark_filename_list is None:
            ...
            abs_path = os.path.abspath(inter_feat_path)
            self.logger.info(f"Loading interaction file: {abs_path}")
            ...
            for filename in self.benchmark_filename_list:
                ...
                if os.path.isfile(file_path):
                    self.logger.info(f"Loading benchmark file: {abs_path}")
                    temp = self._load_feat(file_path, FeatureSource.INTERACTION)
                    self.logger.info(f"  Loaded {len(temp)} interactions from {filename}")
                    ...
                    self.logger.warning(f"Benchmark file not found: {abs_path}")
