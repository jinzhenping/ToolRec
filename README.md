...
python run_recbole.py --model=SASRec --dataset=ml-1m --config_files=SAS_ml1m.yaml --dump_to_chat=True --chat_hislen=50 --seed=2023 --test_v=test
...
### mind_2000 — 프롬프트 피클·평가 TSV 한 번에 만들기 (RecBole 학습 불필요)

`mind_2000.train.inter`, `mind_2000.test.inter`, `mind_2000.item` 과  
`dataset/prompts/test/mind_2000_pattern.json` 이 있으면 아래만 실행하면 됩니다.

```bash
python scripts/build_mind_2000_eval_assets.py
```

생성물:

- `dataset/prompts/test/mind_2000_chat.pkl` — `utils` 가 로드하는 `user_profile` / `item_profile` / `itemID_name`
- `dataset/prompts/test/mind_2000_ui_token.pkl` — 사용자·아이템 토큰 매핑
- `dataset/prompts/test/mind_2000_uid_alias.pkl` — `{user}__inst{k}` → 원 `user_id` (CRS 경로용)
- `dataset/prompts/test/mind_2000_eval_benchmark.tsv` — `evaluate_reranking_mind.py` 입력 형식  
  (`synthetic_uid` `\t` 히스토리 토큰들 `\t` 후보 5개; **첫 후보가 positive**)

옵션: `--history_len 50`(기본), `--max_instances N`(디버그용 일부만).

#reranking
# 전체 사용자 평가 (기본 TSV는 위에서 생성한 mind_2000 벤치마크)
